    ...
    try {
        ...
        const stocks = await page.evaluate(() => {
            ...
            return Array.from(rows)
                ...
                const getNumber = (col) => parseFloat(getText(col)) || 0;
                const getInt = (col) => parseInt(getText(col).replace(/,/g, "")) || 0;
                return {
                ...
                .filter((stock) => stock !== null && stock.trading_code !== "");
        });
        if (!stocks.length) {
        ...
        throw new Error(`Failed to scrape DSE: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
    finally {
        await browser.close();
    }
